...
class OrdersController extends Controller
{
  ...
  return view('admin.Orders.index',$title)->with(['listingOrders'=> $all,'all_count'=>$all_count])->with('result', $result);
        ...
        $orders_products = DB::table('orders_products')
            ->where('admin_id', '=',$admin_id->id)->get();
$count = count($orders_products);
...
return view('admin.Orders.index',$title)->with(['listingOrders'=> $all,'all_count'=>$all_count])->with('result', $result);
 ...
 // return view('admin.Orders.index',$title,compact('all','all_count'));

        // return view("admin.Orders.index", $title)->with('listingOrders', $all);
    ...
    public function vieworder(Request $request)
    {
        ...
        foreach ($order as $data) {
            ...
            $orders_products = DB::table('orders_products')
                ...
                ->select('orders_products.*', 'image_categories.path as image')
                ->where('orders_products.orders_id', '=', $orders_id)
                 ->where('orders_products.admin_id', '=',$admin_id->id)
                ->get();
    }
              //  dd($orders_products);
            $i = 0;
  ...
  $result['commonContent'] = $this->Setting->commonContent();
        return view("admin.Orders.shipping", $title)->with(['listingOrders'=> $all,'all_count'=>$all_count])->with('result', $result);
    }

// afaf gomaa
    public function vieworderforshipping(Request $request)
    {
       // $admin_id = auth()->user();
        ...
        foreach ($order as $data) {
            ...
            $admin_id = auth()->user();
             if ($admin_id->role_id==1) {
               $orders_products = DB::table('orders_products')
                   ->join('products', 'products.products_id', '=', 'orders_products.products_id')
                   ->join('sellerdetails','sellerdetails.users_id','orders_products.admin_id')
                   ->LeftJoin('image_categories', function ($join) {
                       $join->on('image_categories.image_id', '=', 'products.products_image')
                           ->where(function ($query) {
                               $query->where('image_categories.image_type', '=', 'THUMBNAIL')
                   ...
                   ->select('orders_products.*','sellerdetails.*', 'image_categories.path as image')
                   ->where('orders_products.orders_id', '=', $orders_id)
                   ->get();
                        } 
        ...
        $ordersData['currency'] = $this->myVarsetting->getSetting();
        $result['commonContent'] = $this->Setting->commonContent();
        return view("admin.Orders.vieworderforshipping", $title)->with('data', $ordersData)->with('result', $result);
    }
